      ...
      switch (type) {
        case 'number':
          ...
          if (isNaN(chosenNumber) || chosenNumber < 0 || chosenNumber > 36) {
            return interaction.editReply('Invalid number! Choose 0-36');
          }
        ...
        case 'color':
          if (!['red', 'black'].includes(choice)) {
            return interaction.editReply('Invalid color! Choose red or black');
          }
        ...
        case 'parity':
          if (!['even', 'odd'].includes(choice)) {
            return interaction.editReply('Invalid choice! Choose even or odd');
          }
          if (result === 0) won = false;
          else
            won = (choice === 'even' && result % 2 === 0) ||
        ...
        case 'half':
          if (!['1-18', '19-36'].includes(choice)) {
            return interaction.editReply('Invalid choice! Choose 1-18 or 19-36');
          }
          if (result === 0) won = false;
          else
            won = (choice === '1-18' && result <= 18) ||
...
${won ? `You won $${winnings}!` : 'You lost!'}
      ...
      return interaction.editReply('An error occurred while processing your bet. Please try again.');
    }
  }
}
